...
# MAGIC %run /Users/connectmohanms@gmail.com/notebooks/silver_transformation_framework

# COMMAND ----------

# MAGIC %md
# MAGIC # Silver Layer Orchestrator
# MAGIC Main driver for Silver layer transformations
...
dbutils.widgets.text(
    "metadata_path",
    "s3://databricks-workspace-stack-5df43-bucket/unity-catalog/4227733217617218/metadata/silver/",
    "Metadata S3 Path"
)

dbutils.widgets.text(
    "table_list",
    "account_profiles,product_catalog,invoice_details",
...
logger.info("Silver Orchestrator Configuration:")
...
framework = SilverTransformationFramework(
    spark=spark,
    metadata_path=METADATA_PATH
)

logger.info("Silver transformation framework initialized")
...
def get_table_list(table_input: str) -> list:
    ...
    if table_input.lower() == 'all':
        ...
        return ['account_profiles', 'product_catalog', 'invoice_details']
    else:
...
logger.info("=" * 80)
logger.info("SILVER LAYER TRANSFORMATION STARTING")
logger.info("=" * 80)
...
logger.info("=" * 80)
logger.info("SILVER LAYER TRANSFORMATION SUMMARY")
logger.info("=" * 80)
